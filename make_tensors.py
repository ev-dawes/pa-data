...
if __name__ == "__main__":
    ...
    for yield_raster in yields:
        ...
        # Check if all candidates are from the same 100km tile
        # if not, we should probably skip this raster for now.
        # TODO: mosaic the tiles if they are adjacent
        tiles = set([c.properties["grid:code"] for c in candidates])
        if len(tiles) > 1:
            print("Multiple MGRS tiles found in search, skipping")
            continue

        candidate = get_first_candidate_below_threshold(
            candidates, bbox, crs, QA_PERCENT
        )
